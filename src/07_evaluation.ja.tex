...
\paragraph{スループット}
iPerf3を用いたTCPスループット測定を行った（100秒間の連続転送）．
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    ...
    \addplot[fill=GoogleBlue!60] coordinates {(DL（提案）, 15.9) (DL（ベース）, 15.5) (UL（提案）, 5.6) (UL（ベース）, 5.5)};
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{TCPスループットの比較（iPerf3，100秒間転送）}
  \label{fig:throughput_comparison}
\end{figure}

\begin{itemize}
    \item \textbf{下り（Downlink）}: 提案手法 15.9 Mbps／ベースライン 15.5 Mbps
    \item \textbf{上り（Uplink）}: 提案手法 5.6 Mbps／ベースライン 5.5 Mbps
\end{itemize}
測定値はsXGP APのカタログスペック（DL約37Mbps／UL約14Mbps）を下回っているが，これはカタログ値が理想条件（電波環境，変調方式，リソースブロック割当）における理論上限であることに起因する．実環境では，無線区間の電波品質，TCP輻輳制御，およびプロトコルオーバーヘッド等により，実効スループットは理論値の40〜50\%程度となることが一般的である．
...
RTTおよびスループットの測定結果から，提案手法とベースライン環境の間に有意な性能差がないことを確認した．これは，ゲートウェイのGTP-Uヘッダ書き換え処理が通信のボトルネックとなっていないことを示している．したがって，Hybrid Proxy設計（上り方向のTEID変換＋下り方向の透過転送）は，性能オーバーヘッドを抑えつつ機能要件を満たす設計として妥当であったと言える．
...
本研究が採用したアプローチは，4Gと5Gの既存プロトコル仕様を変更せず，両者の間にプロトコル変換層（ゲートウェイ）を挿入するものである．第\ref{chap:proposal}章で示した通り，世代不変な機能要件（AAA，移動管理，U-Plane）に着目し，これらの機能を実現するプロトコル要素間の対応関係（NAS変換，ID変換，TEIDマッピング等）を定義することで，異なる世代間の相互運用を実現した．
...
\begin{itemize}
    \item \textbf{MUST（必須）フィールド}: 世代不変な機能要件（AAA，移動管理，U-Plane）を実現するために必要なパラメータを，すべての世代で共通の形式・エンコーディングで定義する．例えば，ユーザ識別子，認証ベクトル，セッション識別子，QoSパラメータの基本セット等が該当する．
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[scale=0.7, every node/.style={font=\small}]
    ...
    \node[draw, fill=white, anchor=north, font=\scriptsize, inner sep=4pt] at ([yshift=-8pt]current bounding box.south) {
      \begin{tabular}{@{}l@{\hspace{0.6em}}l@{\hspace{0.6em}}l@{\hspace{0.6em}}l@{\hspace{0.6em}}l@{\hspace{0.6em}}l@{}}
      ...
      \csname tikz\endcsname[baseline=-0.6ex]\fill[GoogleYellow!50] (0,0) rectangle (0.18,0.18);\ 衛星/HAPS &
      \csname tikz\endcsname[baseline=-0.6ex]\draw[thick, dashed] (0,0) -- (0.4,0);\ Feeder &
      \csname tikz\endcsname[baseline=-0.6ex]\draw[thick] (0,0) -- (0.4,0);\ Service
      \end{tabular}
    };
  \end{tikzpicture}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[scale=0.72, every node/.style={font=\small}]
    ...
    \draw[<->, thick, dotted, GoogleBlue] (sat-r) -- (sat-r2) node[midway, above, font=\scriptsize, text width=1.2cm, align=center] {ISL\\{\tiny (衛星間)}};
    ...
    % 凡例（下部中央・2行）
    \node[draw, fill=white, anchor=north, font=\scriptsize, inner sep=4pt] at ([yshift=-8pt]current bounding box.south) {
      \begin{tabular}{@{}l@{\hspace{0.6em}}l@{\hspace{0.6em}}l@{\hspace{0.6em}}l@{\hspace{0.6em}}l@{}}
      \csname tikz\endcsname[baseline=-0.6ex]\fill[GoogleLightGray!50] (0,0) rectangle (0.18,0.18);\ Transparent &
      \csname tikz\endcsname[baseline=-0.6ex]\fill[GoogleYellow!50] (0,0) rectangle (0.18,0.18);\ Regenerative &
      \csname tikz\endcsname[baseline=-0.6ex]\fill[GoogleGreen!50] (0,0) rectangle (0.18,0.18);\ 地上側 &
      \csname tikz\endcsname[baseline=-0.6ex]\fill[GoogleBlue!30] (0,0) rectangle (0.18,0.18);\ 地上局/ISL &
      \csname tikz\endcsname[baseline=-0.6ex]\fill[GoogleRed!10] (0,0) rectangle (0.18,0.18);\ 注意点 \\
      \multicolumn{5}{@{}c@{}}{\csname tikz\endcsname[baseline=-0.6ex]\draw[thick, dashed] (0,0) -- (0.4,0);\ Feeder \hspace{0.8em} \csname tikz\endcsname[baseline=-0.6ex]\draw[thick] (0,0) -- (0.4,0);\ Service \hspace{0.8em} \csname tikz\endcsname[baseline=-0.6ex]\draw[thick, dotted, GoogleBlue] (0,0) -- (0.4,0);\ ISL}
      \end{tabular}
    };
  \end{tikzpicture}
