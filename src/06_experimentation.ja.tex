...
\begin{description}
    ...
    \item[R2: プロトコル相互変換] 世代不変な本質的機能（AAA，Mobility Management，U-Plane）を維持しつつ変換
...
\hline
R1: 透過的介在 & eNB/5GCの無改修での接続成立 & シナリオ1 \\
\hline
R2: プロトコル変換 & 世代共通機能（AAA，Mobility Management，U-Plane）の正常動作 & シナリオ1, 2 \\
\hline
R3: 5GC固有機能 & S-NSSAIデフォルト割り当て，QoSマッピング & シナリオ1, 2（部分的） \\
\hline
\end{tabular}
\end{table}

（注）R3については，本プロトタイプがNetwork Slicing等の高度機能を固定値補完で実装しているため（第\ref{chap:design}章参照），完全な機能検証は対象外とする．ただし，S-NSSAIのデフォルト割り当ておよびQCI/5QIのマッピングについては，接続成立を通じて部分的に検証する．

実験の具体的な目的は以下の通りである．

\begin{enumerate}
    \item \textbf{機能検証（R1/R2の実証，R3の部分的実証）}: 4G RAN（sXGP\footnote{shared eXtended Global Platform: 日本国内の1.9GHz帯で免許不要で利用可能なプライベートLTE規格．詳細は第\ref{subsubsec:sxgp}項を参照．}）と5Gコア（Open5GS）が，提案ゲートウェイを介して正常に接続できることを確認する．具体的には，C-Planeにおける登録手順（Registration）とU-Planeにおけるデータ疎通（PDU Session）の成立を通じて，認証やMobility Managementといった「世代共通機能」がプロトコル変換を経ても正しく維持されることを検証する．また，S-NSSAIのデフォルト割り当てやQoSマッピング等，5GC固有機能の一部についても動作を確認する．
...
sXGP（shared eXtended Global Platform）\cite{ARIB_STD_T118}は，日本国内の1.9GHz帯（Band 39）で免許不要で利用可能なプライベートLTE規格であり，TD-LTE（時分割複信方式のLTE）に準拠している．標準的な4G端末およびS1インターフェースをそのまま使用できるため，本研究ではこのsXGPを4G RANとして採用する．
...
本シナリオは，以下の要件を検証する．
\begin{itemize}
    \item \textbf{R1（透過的介在）}: sXGP基地局（無改修のS1インターフェース）とOpen5GS（標準のN2インターフェース）が，ゲートウェイを介して正常に接続できること．
    \item \textbf{R2（プロトコル変換）}: 世代共通機能のうち「AAA」および「Mobility Management」が正しく変換・維持されること．
    \item \textbf{R3（5GC固有機能の整合）}: 登録手順においてS-NSSAI（スライス識別子）のデフォルト割り当てが正常に処理されること（部分的検証）．
\end{itemize}
\begin{itemize}
    \item \textbf{確認項目}:
    \begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{R3（5GC固有機能の整合）}: QoSパラメータ（QCI/5QI）のマッピングが正常に処理されること（部分的検証）．
\end{itemize}
...
\begin{itemize}
    \item \textbf{機能検証（R1/R2の実証，R3の部分的実証）}: sXGP基地局，商用端末（Pixel 8a），Open5GS 5GCを用いた実機環境を構築し，シナリオ1（C-Plane接続検証）においてpcapキャプチャとログ分析により変換規則の成立を確認する．R3についてはS-NSSAIのデフォルト割り当ておよびQoSマッピングの動作を部分的に検証する．
