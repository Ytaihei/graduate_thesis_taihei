...
sXGP（shared eXtended Global Platform）\cite{ARIB_STD_T118}は，日本国内の1.9GHz帯（Band 39）で免許不要で利用可能なプライベートLTE規格であり，TD-LTE（時分割複信方式のLTE）に準拠している．標準的な4G端末およびS1インタフェースをそのまま使用できるため，本研究ではこのsXGPを4G RANとして採用する．
