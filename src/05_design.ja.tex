...
本節では，S1AP\cite{3GPP_TS36413}とNGAP\cite{3GPP_TS38413}間のメッセージ変換ロジックを述べる．
...
表\ref{tab:nas_conversion_ul}および表\ref{tab:nas_conversion_dl}に，4G NAS\cite{3GPP_TS24301}と5G NAS\cite{3GPP_TS24501}の主要なメッセージ変換対応を示す．
...
\begin{figure}[htbp]
  ...
  \end{tikzpicture}
  \caption{プロトタイプにおける接続シーケンス}
  \label{fig:sequence_diagram}
\end{figure}

\subsection{U-Plane変換とTEIDマッピング}
U-Planeでは，eNBとUPF間のユーザデータ転送を仲介する．4GのS1-Uインターフェースと5GのN3インターフェースは共にGTPv1-Uプロトコル\cite{3GPP_TS29281}を使用するため，プロトコル変換は不要であるが，TEIDと宛先IPアドレスの書き換えが必要となる．
...
4GのQCI（QoS Class Identifier）\cite{3GPP_TS23203}と5GのQoS情報（5QI）\cite{3GPP_TS23501_R15}は，数値の定義範囲や付随パラメータが完全には一致しない．
...
\subsection{実装概要}
本プロトタイプはC言語で実装し，約14,000行のソースコードから構成される．主要な外部ライブラリとして，S1AP/NGAPメッセージのASN.1エンコード・デコードにはlibasn1c（asn1c生成コード）を使用した．認証処理のMilenageアルゴリズムおよび鍵導出関数（KDF）の実装は，3GPP TS 35.206\cite{3GPP_TS35206}およびTS 33.501\cite{3GPP_TS33501}の仕様に基づき，Open5GS\cite{open5gs}の実装を参考にした．
...
\item \textbf{QoS Flow}: 5GのFlow単位QoS\allowbreak （GFBR/MFBR）から4GのBearer単位\allowbreak （GBR/MBR）への完全なマッピングは困難である．\ref{subsec:qos_mapping}節で設計したQCI/5QIの対応関係に基づく変換ロジックに対し，本PoCではさらに簡略化してQCI=9固定としている（第\ref{sec:prototype_implementation}節参照）．
\item \textbf{Traffic Steering}: 論理設計ではパケットフィルタに基づく転送先UPF選択を想定しているが，本PoCでは単一UPF構成での検証に限定しており，複数UPFへの動的分岐は未実装である．
\item \textbf{Multi-type PDU Support}: 論理設計ではPDN TypeからPDU Typeへの1対1マッピングを定義しているが，本PoCではIPv4のみを対象としている．IPv6についてはアドレス処理の実装が未完了であり，EthernetおよびUnstructuredタイプは4G RANで生成されないため対象外である．
\item \textbf{セッション変更}: Bearer変更（4G）とPDU Session変更（5G）の相互変換は未実装であり，確立後のセッションパラメータ変更には対応していない．
\item \textbf{IDLE/CONNECTED状態遷移}: S1 Release/Service Request（4G）とN2 Release/Service Request（5G）の変換は未実装である．本PoCはCONNECTED状態での接続確立に焦点を当てており，IDLE状態への遷移および復帰は対象外としている．なお，UEからのService RequestについてはTAU処理で代替しているが，本来のService Request手順には対応していない．
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{プロトコル仕様上の制約}（第\ref{sec:protocol_constraints}節）: 規格差分による制約（S1AP/NGAP間のIE非対称性），手順分離に起因する制約（PDU Session確立待機），機能制限に関する制約（ハンドオーバー，Network Slicing，QoS Flow，Traffic Steering，Multi-type PDU，セッション変更，IDLE/CONNECTED状態遷移），およびセキュリティに関する制約（Ki/OPcアクセス要件）を整理した．
\end{itemize}
