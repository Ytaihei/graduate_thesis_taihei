...
\hline
Downlink (5GC $\rightarrow$ eNB) & N3 TEID \newline （下り：GW側） & (eNB IP, TEID \newline （本実装では書換なし）) \\
\hline
\end{tabularx}
\end{table}

パケット受信時，ゲートウェイはGTP-Uヘッダ内のTEIDを参照して転送処理を行う．UplinkではTEIDマッピングを参照し，GTP-UヘッダのTEIDフィールドを書き換えてUPFへ送出する．Downlinkでは本プロトタイプ実装においてTransparent proxyとしてTEIDは書き換えず，送信先のみeNBのS1-U IPへ切り替えて送出する．転送はOSのネットワークスタックを介して行われるため，外部IP/UDPヘッダは新規に生成され，チェックサムもOSにより計算される．
...
その後，導出したCK, IKを用いて以下の式により4G互換の\textbf{KASME}を生成する（TS 33.401 Annex A.2）．
...
\item \textbf{障害解析用ロギング}: エラー発生時は，関連ID（ENB-UE-S1AP-ID, RAN-UE-NGAP-ID等）やNAS/手順の種別をログに残し，調査の手がかりとする．ただし，ログ形式は一部非統一（printfとlog\_info/log\_error混在）であり，完全な構造化ログではない．
\end{itemize}
