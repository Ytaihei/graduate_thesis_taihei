...
本研究では，モバイル通信の根幹機能である「認証（AAA）」，「登録管理（Registration Management）」，「QoS制御」が，4Gと5Gの世代を超えて本質的に不変であることに着目する．これらの機能はプロトコル表現（メッセージ名やパラメータ形式）こそ異なるものの，状態遷移や目的は共通している．

提案する「S1-N2変換ゲートウェイ」は，この世代不変性を利用して，4G RAN（S1インターフェース）と5G Core（N2/N3インターフェース）の間の差異を吸収・抽象化する．これにより，物理的なRANと論理的なCoreを疎結合にし，RANのハードウェア更新を伴わずにCoreのみを次世代化することを可能にする．

\section{アーキテクチャ設計}
提案システムの全体構成を図\ref{fig:architecture}に示す．ゲートウェイは4G eNBと5G Core（AMF/UPF）の間に配置され，以下の2つのプレーンで変換を行う．
...
\begin{table}[htbp]
\centering
\caption{NASメッセージ変換対応表（上り：UE→Core）}
...
\begin{table}[htbp]
\centering
\caption{NASメッセージ変換対応表（下り：Core→UE）}
...
IoT-NTNの移行期においても，本変換器はS1AP↔NGAPおよびS1-U↔N3の\textbf{コア境界}で動作するため，NB-IoT/EPCに依存する既存環境から5GCへ\textbf{漸進的に移行}する際の検証足場として有効である．無線特性（長RTT，Doppler，間欠接続など）は本研究の対象外だが，\textbf{手順整合性・状態管理・タイマ設定}の妥当性を高遅延条件下で検証するための土台を提供する．
