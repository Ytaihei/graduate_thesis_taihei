...
本章では，第\ref{chap:background}章で述べた技術背景を踏まえ，4G RANを改修なしに5Gコア（5GC）へ収容可能にするS1-N2変換ゲートウェイを提案する．本節では，提案手法の根拠となる2つの重要な分析フレーム---「機能的共通性」と「5GC固有機能の階層的分析」---を述べる．

\subsection{4G/5G間の機能的共通性 (Functional Commonality)}
\label{subsec:functional_commonality}

モバイル通信システムは世代ごとにプロトコル仕様が大きく異なるが，提供される基本的な制御機能の役割（Role）には高い共通性が存在する\cite{Watanabe_ADMobile2025}．本手法は，このプロトコル形式の違いを乗り越え，「機能的な等価性」に基づいて相互運用を実現するものである．

\subsubsection{制御プレーン機能の対応関係}

CS（Circuit Switched）領域が廃止されオールIP化された4G以降，制御プレーン（C-Plane）の主要機能は「移動管理（Mobility Management: MM）」と「セッション管理（Session Management: SM）」に大別される．

\begin{itemize}
\item \textbf{移動管理 (MM)}: 端末の位置登録，認証，およびハンドオーバー制御を行う機能．4GのMMEが担っていた役割は，5GではAMFに継承されている．両者ともNAS（Non-Access Stratum）プロトコルの終端点として機能し，ステートマシン（例: EMM-REGISTERED $\leftrightarrow$ 5GMM-REGISTERED）にも明確な対応関係が存在する．
\item \textbf{セッション管理 (SM)}: データ転送経路（ベアラ/フロー）の確立・解放を行う機能．4GではMMEとS-GW/P-GWが連携して行っていたが，5GではSMFとして独立したNFに集約されている．手続き上の差異はあるものの，「IPアドレスを割り当て，トンネルを設定する」という基本的動作は共通している．
\end{itemize}

\subsubsection{ユーザプレーン機能の対応関係}

ユーザデータの転送を行うユーザプレーン（U-Plane）においても，4Gと5Gは共にGTP-U（GPRS Tunneling Protocol User Plane）プロトコルを採用しており，基本的な転送メカニズムは維持されている．

4GのS-GW/P-GWと5GのUPFは，共にパケットのルーティング，カプセル化/非カプセル化，および課金情報の収集を行うという点で機能的に等価である．したがって，トンネルID（TEID）の管理方法さえ整合させれば，データパケットの相互運用は比較的容易に実現可能である．

この根本的な「機能の共通性」が存在するからこそ，以下で述べるようなプロトコル変換によるインターワーキングが可能となる．
...
\subsubsection{基盤層 (Architecture / Enabler)}
...
\subsubsection{制御メカニズム層 (Mechanism)}
...
\subsubsection{応用サービス層 (Service / Outcome)}
...
本実装では，共通機能の検証に集中するため，本層の機能は初版では部分的なサポートとし，プロトタイプの段階を超えた次段階で完全なサポートを想定する．

\section{基本概念：世代共通機能に基づく疎結合化}
\label{sec:basic_concept_v2}
