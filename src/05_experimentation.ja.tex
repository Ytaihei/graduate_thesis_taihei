...
トポロジは，\texttt{UE}—\texttt{eNB(sXGP)}—(S1AP,S1-U)→\texttt{Converter}—(NGAP,N3)→\texttt{5GC} の直列構成である．Converterは制御面（S1AP/NGAP）とユーザ面（S1-U/N3）で論理IFを分ける．5GC側は，AMF/SMF/UPFを同一ホスト内に配置する．UEアドレスプールやAPN等はOpen5GSのデフォルト方針に準拠し，必要に応じてIPv6-only + NAT64/464XLAT, DNS64 構成を併用する（第\ref{chap:related}章；参考: \cite{rfc6146,rfc6147,rfc6877}）．
...
実UEを用いることで，\textbf{シミュレータでは顕在化しにくい相互運用性問題や端末OS依存の振る舞い}（例：NAS再送シーケンスのズレ，IPv6-only下でのAPI失敗，バックグラウンド時の接続断）を早期に発見し，\textbf{パケットキャプチャと再現手順}を添えてOSSプロジェクトへフィードバックできる．

\subsection{NTN伝搬条件のエミュレーション}
\paragraph{目的} IoT-NTNを想定した高遅延・間欠性・損失環境で，\textbf{手順整合性・再送/タイマ挙動・安定性}を評価する．
\paragraph{手順}
\begin{itemize}
	\item ネットワークに遅延/ジッタ/損失/一時断を注入（例: netem/tc）．往復遅延は衛星リンク想定の数百ms～秒オーダまで可変．
	\item 登録・PDUセッション確立・Keepalive・再登録（Idle復帰）を繰り返し実行し，成功率・所要時間・再送回数を計測．
	\item S1AP/NGAP/NASおよびGTP-Uのpcap，AMF/SMF/UPFログ，Converterログを収集し，時系列で突合する．
\end{itemize}
\paragraph{観測と成果物} 成功率，タイムアウト/原因コード，再送回数，RTT/ジッタ，スループットの変動をCSV/JSONで保存する．既存の高遅延系シナリオ（例：\texttt{results/mars\_distance\_*}）をベースにプロファイル化し，回帰セットとして再実行可能にする．
