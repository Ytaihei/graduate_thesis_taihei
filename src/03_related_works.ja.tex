...
5G CoreのOSS実装としては，Open5GS，free5GC，OpenAirInterface（OAI）などが広く用いられている．Open5GSは4G EPC/5G Coreを統合的に実装し，Dockerコンテナでの展開が容易である．WebUIによる加入者管理や，AMF/SMF/UPF等の機能分離が明確で，研究用途に適する．本研究では，Open5GSを基盤として5GCを構築し，sXGP-5Gコンバータとの相互接続検証を行った（\S\ref{chap:experiment}）．free5GCやOAIのプロジェクトも活発に更新されており，機能比較や相互運用評価に有用である\cite{free5gc,oai}．
...
RAN側のOSSとしては，srsRAN（旧srsLTE）とUERANSIMが代表的である．srsRANは4G eNB/UEに加えて5G gNB/UE（Project系）も提供し，ZMQベースのRFシミュレーションによって無線機器なしにプロトコル検証が可能である．UERANSIMは5GのgNB/UEシミュレータであり，NGAP/NASの制御面検証に適する\cite{srsran,ueransim}．本研究では，srsRANのZMQ構成を用い，4G/5G双方の接続性とデータ疎通を確認している（TESTING\_RESULTS参照）．
...
\section{IoT-NTNの現状とEPC依存，5GC移行の課題}
IoT-NTN（Non-Terrestrial Network, 衛星等の非地上系）では，現行の運用・実装がNB-IoT/EPCを前提とするケースが多く（例: TS~\cite{threegpp-23401}），端末・RAN・コアの三者を同時に5GCへ移行することは現実的に難しい．端末の長寿命サイクル，無線特性（長RTT・間欠接続）に基づく制御面最適化，運用コスト等の制約により，\textbf{漸進的な移行経路}が求められる．一方，5GS側のNASや手順はTS~\cite{threegpp-24501}で規定されており，コア境界での相互接続検証は移行設計の観点で重要である．

この文脈において，コア境界での\textbf{相互接続・変換・段階的置換}は有効なアプローチとされる．NB-IoT/EPCに依存する要素を保持しつつ，5GC側の受け皿（AMF/SMF/UPF等）を先行準備し，RANや端末の更新とは独立に\textbf{手順整合性の検証と運用準備}を進めることで，移行期のリスクを抑えられる．本研究の環境（S1AP↔NGAP，S1-U↔N3のブリッジ）は，この種の漸進移行における\textbf{検証基盤}として機能しうる．ただし，NTN固有の無線条件（長RTTやDoppler，間欠性）への最適化は無線/RAN側の課題であり，本研究のスコープ外である点を明確にしておく．
...
sXGPの利点は，法令遵守の範囲で迅速な反復実験が可能である点にある．一方，チャネル幅10MHz・送信電力上限といった物理的制約が存在し，高スループット評価や屋外広域での検証には適さない．研究計画においては，sXGPを基盤とした実機検証と，ZMQ/RFシミュレーションによる反復試験を適切に使い分けることが重要である．本研究は，両者を組み合わせることで，実装課題発見のための検証サイクルを構築した点に特徴がある．
