...
% タイトル情報
\title{世代不変性に基づくレガシーRANの5GC収容アーキテクチャ}
...
\begin{frame}{学術的貢献と実用的インパクト}
  \begin{block}{学術的貢献}
    \begin{itemize}
      ...
      \item RANとCNの進化サイクルを分離する新しい移行モデルの提示
    \end{itemize}
  \end{block}
  \begin{block}{実用的インパクト}
    \begin{itemize}
      \item \textbf{R\&D:} 低コストな次世代CN検証環境(既存設備の再利用)
      \item \textbf{商用展開:} RAN更新を伴わない柔軟なCN先行移行戦略
...
\begin{frame}{今後の展望と未実装機能}
  \begin{itemize}
    \item \textbf{モビリティ管理の完全対応}
      \begin{itemize}
        \item Handover (S1ハンドオーバー $\leftrightarrow$ N2ハンドオーバー変換)
        \item Paging (Idleモードからの着信・復帰)
      \end{itemize}
    \item \textbf{セッション管理の高度化}
      \begin{itemize}
        \item Service Request (Idleからのデータ再開)
        \item Multi-PDU Session (複数スライス/ベアラの同時利用)
        \item Detach / Deregistration (明示的な切断手順)
      \end{itemize}
    \item \textbf{セキュリティ機能の拡張}
      \begin{itemize}
        \item NAS Ciphering (現在は完全性保護のみ、暗号化はNull)
      \end{itemize}
    \item \textbf{NTN環境での実証}
      \begin{itemize}
        \item 衛星回線エミュレータを用いた長遅延・ロス耐性評価
      \end{itemize}
  \end{itemize}
\end{frame}
